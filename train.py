...
list_features = [feature for feature in list_features if not feature in remove_list]
...
for (idnum, features, tags) in collected_data:
  features.update(utilities.bag_of_words(idnum_dict[idnum]))
  if sum(tags) == 0:
...
for i in range(6):
	...
	classifier = nltk.classify.NaiveBayesClassifier.train(train_set) #choose Naive Bayes classification
	classifiers[i] = classifier
	evaluate(classifier, test_set, 'test set', labels[i])
pickle.dump(classifiers,open('store_classifier.txt','wb'))
count = []
for (feature, tags) in test_data:
	predict = []
	for i in range(6):
		predict.append(classifiers[i].classify(feature))
	same = 0
	different_true = 0
	different_false = 0
	for i in range(6):
		if predict[i] == tags[i]:
			...
			if predict[i] == 0:
				...
				different_true += 1
	count.append((same, different_false, different_true))
fd = nltk.FreqDist(count)
print("Accuracy: {:.4f}".format(fd[(6,0,0)]/len(test_data)))
